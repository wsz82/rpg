...
public class Geometry {
    ...
    public static boolean isInside(List<Coords> polygon, Coords p, int maxX) {
        int n = polygon.size();
        if (n < 3) {
            return false;
        }

        Coords extreme = new Coords(maxX, p.y);
        ...
        do {
            ...
            inner: if (doIntersect(pi, pNext, p, extreme)) {
                if (orientation(pi, p, pNext) == 0) {
                    return onSegment(pi, p,
                            pNext);
                }
    ...
    private static boolean doIntersect(Coords p1, Coords q1,
                               Coords p2, Coords q2) {
