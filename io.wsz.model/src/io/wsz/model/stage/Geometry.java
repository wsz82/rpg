...
public class Geometry {
    ...
    public static boolean ovalIntersectsPolygon(Coords c, CreatureSize s, List<Coords> polygon) {
        ...
        for (int i = 0; i < pSize; i++) {
            ...
            if (pSize == 1) return isInsidePolygon(first, polygon);
        ...
        for (List<Coords> rawIPolygon : iPolygons) {
            ...
            for (List<Coords> rawOPolygon : oPolygons) {
                ...
                for (int ic = 0; ic < iSize; ic++) {
                    ...
                    if (iSize == 1) return isInsidePolygon(i1, oPolygon);
                    ...
                    for (int oc = 0; oc < oSize; oc++) {
                        ...
                        if (oSize == 1) return isInsidePolygon(o1, iPolygon);
    ...
    static boolean onSegment(Coords p, Coords q, Coords r) {
    ...
    static int orientation(Coords p, Coords q, Coords r) {
    ...
    public static boolean isInsidePolygon(Coords pos, List<Coords> polygon) {
        int n = polygon.size();
        if (n < 3) {
            return false;
        }
        ...
        do {
            ...
            inner: if (doIntersect(pi, pNext, pos, EXTREME)) {
                if (orientation(pi, pos, pNext) == 0) {
                    return onSegment(pi, pos,
                            pNext);
                }
